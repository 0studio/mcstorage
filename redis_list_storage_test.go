...
func TestGetLimitRedis(t *testing.T) {
	...
	redisListStorage.Set(key.String("1"), slice)
	result, _ := redisListStorage.Getlimit(key.String("1"), 0, 0, 1, 20)
	defer redisListStorage.Delete(key.String("1"))
...
func TestAddItemRedis(t *testing.T) {
	...
	result, _ = redisListStorage.Getlimit(key.String("1"), 0, 200, 1, 20)
	if len(result.(IntReversedSlice)) != 20 {
		t.Error("len should be 20")
	}
	...
	redisListStorage.AddItem(key.String("1"), 201)
	result, _ = redisListStorage.Getlimit(key.String("1"), 0, 0, 1, 20)
	if len(result.(IntReversedSlice)) != 20 {
		t.Error("len should be 20")
	}
	...
	redisListStorage.DeleteItem(key.String("1"), 193)
	result, _ = redisListStorage.Getlimit(key.String("1"), 0, 0, 1, 20)
	if len(result.(IntReversedSlice)) != 20 {
		t.Error("len should be 20")
	}
